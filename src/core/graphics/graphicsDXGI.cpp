...
int core_graphics_render(struct core_window_Window *window, struct core_graphics_obj* graphicsObj, struct core_graphics_shader* shaderObj, float* cubePositions[]) {
    ...
    window->swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&dxColorBuffer);

	
    ID3D11RenderTargetView* colorBufferView;
	CD3D11_RENDER_TARGET_VIEW_DESC rtvD = CD3D11_RENDER_TARGET_VIEW_DESC(D3D11_RTV_DIMENSION_TEXTURE2D, DXGI_FORMAT_R8G8B8A8_UNORM);
    window->device->CreateRenderTargetView(
      dxColorBuffer,
      &rtvD,
      &colorBufferView);

	PFNWGLDXREGISTEROBJECTNVPROC _wglDXRegisterObjectNV = (PFNWGLDXREGISTEROBJECTNVPROC)wglGetProcAddress("wglDXRegisterObjectNV");
	HANDLE rtvHandleGL = _wglDXRegisterObjectNV(window->gl_handleD3D, dxColorBuffer, window->rtvNameGL, GL_TEXTURE_2D, WGL_ACCESS_READ_WRITE_NV);
	...
	glm_translate(graphicsObj->view, v);
	...
	for (unsigned int i = 0; i < 10; i++)
	{
		...
		glm_rotate(graphicsObj->model, glm_rad(angle), axis);
